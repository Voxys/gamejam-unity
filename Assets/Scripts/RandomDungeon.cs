...
public class RandomDungeon : MonoBehaviour
{
    ...
    {
        Sprite t_Sprite = Wall.GetComponent<SpriteRenderer>().sprite;
        float t_Scale = 1 / t_Sprite.bounds.size.x;
        Wall.transform.localScale = new Vector3(t_Scale, t_Scale, t_Scale);
        //Vector3 t_pos = new Vector3(i, j, 0);
        Vector3 t_pos = new Vector3(i + 0.5f, j + 0.5f, 0);
        Instantiate(Wall, t_pos, Quaternion.identity);
    }
